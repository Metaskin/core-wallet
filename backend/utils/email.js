...
const FROM = process.env.EMAIL_FROM
  || (useSmtp
      ? `MCT Bank <${process.env.SMTP_USER}>`
      : 'MCT Bank <onboarding@resend.dev>');
...
if (useSmtp) {
  ...
  if (freeMailDomains.includes(domain)) {
    console.warn(`[Email] WARNING: Resend cannot send from ${fromAddr} — ${domain} is not a Resend-verified domain. All sends will fail. Set SMTP_HOST/SMTP_USER/SMTP_PASS to use SMTP instead, or set EMAIL_FROM=MCT Bank <onboarding@resend.dev>.`);
  }
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .header{background:linear-gradient(135deg,#001540 0%,#003087 100%);padding:32px;text-align:center}
    .logo-box{display:inline-flex;align-items:center;justify-content:center;width:44px;height:44px;background:rgba(255,255,255,0.12);border:1.5px solid rgba(255,255,255,0.25);border-radius:12px;margin-bottom:12px}
    ...
    .body{padding:32px}
    .eyebrow{color:#93b4e0;font-size:11px;text-transform:uppercase;letter-spacing:1.2px;font-weight:600;margin-bottom:10px}
    ...
    .btn{display:inline-block;background:#003087;color:#fff;text-decoration:none;padding:13px 28px;border-radius:10px;font-weight:600;font-size:15px}
    ...
    .success-box{background:rgba(0,48,135,0.08);border:1px solid rgba(0,48,135,0.30);border-radius:8px;padding:12px 14px;color:#93b4e0;font-size:12px;line-height:1.6;margin-bottom:16px}
    .amount{color:#93b4e0;font-size:28px;font-weight:800;letter-spacing:-0.5px;font-family:'Courier New',monospace}
  ...
  <div class="outer">
    <div class="card">
      <div class="header">
        ...
        <div class="brand">MCT Bank</div>
      </div>
      <div class="body">${bodyHtml}</div>
      <div class="footer">
        © 2024 Metropolitan Capital &amp; Trust Bank · This is an automated security message. Do not reply.<br>
        If you need help, contact <a href="mailto:support@mctbank.online" style="color:rgba(255,255,255,0.35);text-decoration:none">support@mctbank.online</a>
      </div>
    </div>
  </div>
</body>
...
async function sendLoginAlertEmail(to, { name, ip, device, time }) {
  ...
  const body = `
    ...
    <p class="para">Hi ${displayName}, we detected a new sign-in to your MCT Bank account. Here are the details:</p>
  ...
  const text = [
    `MCT Bank Security Alert`,
    ``,
    `Hi ${displayName}, a new sign-in was detected on your MCT Bank account.`,
    ``,
    `Time:    ${displayTime}`,
    `IP:      ${ip}`,
    `Device:  ${shortDevice}`,
    ...
    `If this wasn't you, change your password immediately and contact support@mctbank.online.`,
  ].join('\n');

  await send({ to, subject: 'New sign-in to your MCT Bank account', html: layout(body), text });
}
...
async function sendPasswordResetEmail(to, resetUrl) {
  ...
  await send({ to, subject: 'Reset your MCT Bank password', html: layout(body), text });
}
...
async function sendTransactionEmail(to, { name, type, amount, currency, direction, counterparty, balance }) {
  ...
  const amountColor   = isCredit ? '#93b4e0' : 'rgba(255,255,255,0.75)';
  ...
  const body = `
    ...
    <div class="success-box">✓ This transaction has been recorded. View your full history in the MCT Bank app.</div>
  `;

  const text = [
    `MCT Bank Transaction Alert`,
    ``,
    ...
    ``,
    `View your full transaction history in the MCT Bank app.`,
  ].filter(Boolean).join('\n');

  await send({ to, subject: `MCT Bank: You ${verb} ${displayAmount}`, html: layout(body), text });
}
...
async function sendOtpEmail(to, code) {
  const body = `
    ...
    <p class="para">Use the code below to complete your MCT Bank sign-in. Enter it within 10 minutes.</p>
    <div style="text-align:center;margin:24px 0">
      <div style="display:inline-block;background:rgba(0,48,135,0.10);border:1.5px solid rgba(0,48,135,0.30);border-radius:12px;padding:20px 36px">
        <span style="font-size:40px;font-weight:800;letter-spacing:12px;color:#93b4e0;font-family:'Courier New',monospace">${code}</span>
      </div>
    </div>
    <div class="notice">⏱ This code expires in <strong>10 minutes</strong>.</div>
    <div class="alert">🔒 <strong style="color:#fff">Security notice:</strong> If you did not attempt to sign in, ignore this email. Never share this code with anyone, including MCT Bank support.</div>
  `;

  const text = [
    `Your MCT Bank login code: ${code}`,
    ``,
    `This code expires in 10 minutes.`,
    `If you did not request this, ignore this email.`,
  ].join('\n');

  await send({ to, subject: 'Your MCT Bank verification code', html: layout(body), text });
}
