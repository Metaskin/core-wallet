...
export default function TransactionModal({ transaction: tx, accountNumber, open, onClose, onDeleted }) {
  ...
  const incoming     = resolveDirection(tx, accountNumber) === 'in';
  const amountPrefix = incoming ? '+' : '−';
  const failed       = tx.status === 'failed';
  const amountColor  = failed ? 'text-bank-red' : incoming ? 'text-bank-green' : 'text-gray-900';

  const { merchantName, txType, subDetail } = classifyTransaction(tx);
  const isTransfer = tx.type === 'transfer';

  // For transfers, resolve the other party's account name from the seeded description
  const transferCounterparty = isTransfer
    ? (tx.description || '').replace(/^Transfer (?:to|from) /i, '').trim() || null
    : null;
  ...
  const handleDownload = () => {
    const lines = [
      ...
      subDetail ? `Details     : ${subDetail}` : '',
      isTransfer && tx.sender?.accountNumber   ? `From Acc.   : ${tx.sender.accountNumber}`   : '',
      isTransfer && tx.receiver?.accountNumber ? `To Acc.     : ${tx.receiver.accountNumber}` : '',
      '───────────────────────────────────────',
      ...
      >
        <div className="bg-white sm:rounded-2xl overflow-hidden shadow-card-lg rounded-t-2xl">
          ...
          <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-100">
            <div className="flex items-center gap-3">
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                failed ? 'bg-red-50' : incoming ? 'bg-emerald-50' : 'bg-blue-50'
              }`}>
                {failed ? <FailIcon /> : incoming ? <ArrowDownIcon /> : <ArrowUpIcon />}
              </div>
              <div>
                <p className="text-gray-800 font-semibold text-sm">{merchantName}</p>
                <p className="text-gray-400 text-xs">{txType}</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="px-5 py-3 border-b border-gray-100 space-y-0">
            ...
            <Row label="Type">
              <span className="text-gray-800 text-sm">{txType}</span>
            </Row>
            <Row label="Merchant">
              <span className="text-gray-800 text-sm font-medium">{merchantName}</span>
            </Row>
            {subDetail && (
              <Row label="Details">
                <span className="text-gray-600 text-sm">{subDetail}</span>
              </Row>
            )}
            {isTransfer && transferCounterparty && (
              <>
                {incoming ? (
                  <Row label="From"><span className="text-gray-800 text-sm">{transferCounterparty}</span></Row>
                ) : (
                  <Row label="To"><span className="text-gray-800 text-sm">{transferCounterparty}</span></Row>
                )}
              </>
            )}
            ...
            <Row label="Date">
              <span className="text-gray-800 text-sm">{formatDate(tx.createdAt)}</span>
            </Row>
            <Row label="Reference">
              <span className="text-gray-400 text-xs font-mono">{tx.reference}</span>
            </Row>
