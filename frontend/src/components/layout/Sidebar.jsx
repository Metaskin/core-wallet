...
// mobileOpen / onMobileClose are only used on < md breakpoint.
// On desktop the sidebar is always visible and these props are ignored.
export default function Sidebar({ mobileOpen = false, onMobileClose = () => {} }) {
  ...
  return (
    <aside
      className={[
        // Shared layout
        'flex flex-col bg-surface-2 border-r border-white/[0.06] z-50',
        // Mobile: fixed slide-in drawer (width slightly wider for thumb comfort)
        'fixed inset-y-0 left-0 w-72',
        'transition-transform duration-300 ease-in-out',
        mobileOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full',
        // Desktop: static sticky column, translate reset, narrower
        'md:static md:translate-x-0 md:shadow-none',
        'md:w-60 md:h-screen md:sticky md:top-0 md:shrink-0',
      ].join(' ')}
    >
      {/* ── Logo + notification bell ─────────────────────────────────────── */}
      <div className="px-6 pt-7 pb-6 flex items-center gap-3 shrink-0">
        <div className="w-8 h-8 rounded-lg bg-accent flex items-center justify-center text-white font-bold text-sm shrink-0">C</div>
        <span className="font-display font-bold text-white text-lg tracking-tight">CoreWallet</span>

        {/* Bell is on the desktop sidebar header; mobile has it in the top bar */}
        <button
          onClick={panelOpen ? closePanel : openPanel}
          title="Notifications"
          className={`ml-auto hidden md:flex relative p-1.5 rounded-lg transition-colors ${
            panelOpen
        ...
        <button
          ...
          className="ml-auto md:hidden p-1.5 rounded-lg text-white/30 hover:text-white/70 hover:bg-white/5 transition-colors"
        >
          <XIcon />
        </button>
      </div>
