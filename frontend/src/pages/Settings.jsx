...
export default function Settings() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-2xl w-full">
...
function TransactionPinSection() {
  ...
  return (
    <>
      <Section title="Transaction PIN" description="Required when viewing full card details" icon={<ShieldIcon />}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            {hasPin === null ? (
              <div className="w-2 h-2 rounded-full bg-gray-300 animate-pulse" />
            ) : hasPin ? (
              <div className="w-2 h-2 rounded-full bg-bank-green" />
            ) : (
          ...
          <button onClick={() => setShowSetModal(true)} className="btn-primary text-sm py-2 px-4">
            {hasPin ? 'Change PIN' : 'Set PIN'}
          </button>
        </div>
        {!hasPin && hasPin !== null && (
          <p className="text-bank-amber text-xs mt-3">
...
function ChangePasswordSection({ changePassword }) {
  ...
  const update = (field) => (e) => { setForm(f => ({ ...f, [field]: e.target.value })); setSuccess(false); };
  ...
  return (
    <Section title="Change Password" description="Use a strong password you don't use anywhere else" icon={<LockIcon />}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Field label="Confirm New Password">
          ...
          {form.confirm && form.newPassword !== form.confirm && (
            <p className="text-bank-red text-xs mt-1.5">Passwords do not match</p>
          )}
        </Field>
        <div className="flex items-center gap-3 pt-1">
          ...
          {success && (
            <span className="text-bank-green text-sm flex items-center gap-1.5 animate-fade-in">
              <CheckIcon /> Saved
            </span>
          )}
        </div>
...
function ChangeEmailSection({ user, changeEmail }) {
  ...
  return (
    <Section title="Change Email" description="Your current email is used to sign in" icon={<MailIcon />}>
  ...
  return (
    <div className="bank-card p-6 animate-fade-up">
      <div className="flex items-start gap-3 mb-5">
        <div className="w-9 h-9 rounded-xl bg-[#EEF4FF] flex items-center justify-center text-navy shrink-0 mt-0.5">
          {icon}
        </div>
        <div>
      ...
      <div className="flex gap-1 mb-1">
        {[0, 1, 2, 3, 4].map(i => (
          <div
            ...
            style={{ backgroundColor: i < score ? lvl.color : '#E5E7EB' }}
          />
        ))}
      </div>
      <p className="text-[10px]" style={{ color: lvl.color }}>{lvl.label}</p>
    </div>
  );
}
